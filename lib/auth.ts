  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        console.log('=== 登录认证开始 ===')
        console.log('收到的邮箱:', credentials?.email)

        if (!credentials?.email || !credentials?.password) {
          console.log('认证失败: 邮箱或密码为空')
          return null
        }

        console.log('查询用户:', credentials.email)
        ...
        if (!user.password) {
          console.log('认证失败: 用户没有设置密码（可能是OAuth用户）')
          return null
        }
